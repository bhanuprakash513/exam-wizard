	 ...
	 <div><br /><br /><br /><br /></div>
		...
		<div id="quizList">
			    ...
			    <tbody>
			    	<c:if test="${quizList!=null && quizList.size()>0}">
						<c:forEach items="${quizList}" var="quiz">
							<tr>
								<td><a href="${pageContext.request.contextPath}/quiz/viewResult/${quiz.id}">${quiz.title}</a><br></td>
					            <td>${quiz.type}</td>
			...
			<div id="pagination">
				<div id="page_next">
					<c:if test="${prev}">
						<a  class="next_page" href="${pageContext.request.contextPath}/quiz/quizzesTaken?page=${prevPage}">Prev</a>
					</c:if>
				</div>
			
				<div  id="page_prev">
					<c:if test="${next}">
						<a  class="next_page" href="${pageContext.request.contextPath}/quiz/quizzesTaken?page=${nextPage}">Next</a>
					</c:if>
				</div>
			</div>
